  ...
  class Append < SPARQL::Algebra::Operator::Binary
    ...
    def execute(queryable, solutions:, **options)
      @solutions = RDF::Query::Solutions(solutions.map do |solution|
        ...
        raise TypeError, "operand is not a list: #{list.to_sxp}" unless list.list? && list.valid?
        raise TypeError, "operand is not a list of lists: #{list.to_sxp}" unless list.to_a.all? {|li| li.list?}
